...
class googleimagesdownload:
    ...
    def download_image(self, image_url, image_format, main_directory, dir_name, count, print_urls, socket_timeout,
        ...
        if ignore_urls:
            if any(url in image_url for url in ignore_urls.split(',')):
                return "fail", "Image ignored due to 'ignore url' parameter", None, image_url, None
        if thumbnail_only:
            return "success", "Skipping image download...", str(image_url[(image_url.rfind('/')) + 1:]), image_url, None
        if no_download:
            return "success", "Printed url without downloading", None, image_url, None
        try:
            req = Request(image_url, headers={
                "User-Agent": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"})
            try:
                ...
                if type == "image/jpeg" or type == "image/jpg":
                    ...
                    return_image_name = ''
                    absolute_path = ''
                    return download_status, download_message, return_image_name, absolute_path, None
        ...
        return download_status, download_message, return_image_name, absolute_path, img
    ...
    def _get_all_items(self, image_objects, main_directory, dir_name, limit, arguments):
        ...
        while count < limit + 1 and i < len(image_objects):
            try:
                ...
                # to be contrary to the implementation details. 
                    ...
                    # download the images
                    download_status, download_message, return_image_name, absolute_path, img = self.download_image(
        ...
        return items, errorCount, abs_path, images
    ...
    def download(self, arguments):
        ...
        if __name__ != "__main__":
            ...
            if 'config_file' in arguments:
                ...
                total_errors = 0
                for rec in records:
                    paths, errors, images = self.download_executor(rec)
                    for i in paths:
                        paths_agg[i] = paths[i]
                    if not arguments["silent_mode"]:
                ...
                return paths_agg, total_errors, images
            # if the calling file contains params directly
            else:
                paths, errors, images = self.download_executor(arguments)
                for i in paths:
                    paths_agg[i] = paths[i]
                if not arguments["silent_mode"]:
                    if arguments['print_paths']:
                        print(paths.encode('raw_unicode_escape').decode('utf-8'))
                return paths_agg, errors, images
    ...
    def download_executor(self, arguments):
        ...
        for pky in prefix_keywords:  # 1.for every prefix keywords
            for sky in suffix_keywords:  # 2.for every suffix keywords
                ...
                while i < len(search_keyword):  # 3.for every main keyword
                    ...
                    items, errorCount, abs_path, images = self._get_all_items(images, main_directory, dir_name, limit,
        ...
        return paths, total_errors, images
